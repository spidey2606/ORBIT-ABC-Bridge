...
(function () {
  ...
  function buildToolbar() {
    ...
    useBtn.addEventListener('click',       () => { pasteResponse(document.getElementById('ob-resp-text').value); showFetchPane(); });
    retestBtn.addEventListener('click',    () => { showFetchPane(); onFetchClick(); });
    discardBtn.addEventListener('click',   () => { showFetchPane(); setStatus('Ready \u2014 select a customer message', ''); });
  }
  ...
  if (IS_BA) {
    ...
    window.__bcSSE = { done: null, start: null };
      ...
      window.EventSource = function(url, init) {
        ...
        inst.addEventListener('error', function() {
          if (inst.readyState === 2) {
            ...
            console.log('[ORBIT Bridge] SSE stream closed (DONE)');
          }
        });
        return inst;
      };
    ...
    try {
      ...
      if (mode === 'single') {
        ...
        for (let i = 0; i < queries.length; i++) {
          ...
          if (i < queries.length - 1) await sleep(QUERY_GAP);
        }
      }

      if (!finalResponse) throw new Error('BC response was empty');
      let detailsWait = 0;
      while (detailsWait < 300) {
        const bubs = document.querySelectorAll(SEL.ASSISTANT_MSG);
        const lb = bubs[bubs.length - 1];
        const lbc = getBubbleContainer(lb);
        if (lbc && lbc.querySelector('details')) break;
        await sleep(200);
        detailsWait += 200;
      }
      const sourceLinks = extractSourceLinks();
      console.log('[ORBIT Bridge] source links found:', sourceLinks.length, '(waited', detailsWait, 'ms)');
  ...
  function isLoadingPhrase(text) {
    const t = text.trim();
    if (t.length > 200) return false;
    return t.split('\n').some(line => {
      const l = line.trim();
      return l.length > 0 && l.length < 120 && BA_LOADING_RE.some(p => p.test(l));
    });
  }
  ...
  function captureResponse(panel, query, inputEl, bubbleCountBefore) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const doFinish = (reason) => {
        ...
        panelBefore = panel?.innerText ?? '';
        const result = getNewBubbleText() || diffResponse('', before, query);
      ...
      const iv = setInterval(() => {
        ...
        if (window.__bcSSE?.done > queryStartTime && silenceMs >= 500) {
          console.log('[ORBIT Bridge] SSE stream closed \u2014 BC done (sse-done)');
          doFinish('sse-done');
          return;
        }
